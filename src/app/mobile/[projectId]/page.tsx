...
export default function MobileProjectView() {
  ...
  return (
      ...
      bottomBar={
        <nav className="flex-shrink-0 bg-surface-topbar border-t border-border-default flex">
          {TABS.map((tab) => (
            <button
              key={tab.id}
              onClick={() => { setActiveTab(tab.id); setFocusTaskId(null); setIsNewTask(false); }}
    ...
    >
      {activeTab === 'streams' && (
        <MobileStreamView tasks={tasks} projectId={projectId} onTaskCreated={fetchTasks} focusTaskId={focusTaskId} isNewTask={isNewTask} />
      )}
      {activeTab === 'board' && (
